    ...
    stage('Build') {
      ...
      post {
        success {
          ...
          slackSend channel: 'rally-jenkins-integration', color: '#00ff00', message: 'SUCCESSFUL' "JOB NAME = ${env.JOB_NAME} BUILD # =  ${env.BUILD_NUMBER}
          BUILD URL = ${env.BUILD_URL} ", teamDomain: 'rallydemo', tokenCredentialId: 'JenkinsSlack'
        }
      }
    }
  }
